...
const (
	...
	// CheckpointRestoreRuntimeConfigAnnotation stores non-secret restore-time
	// Dynamo runtime configuration projected into /etc/podinfo. CRIU restores
	// the checkpoint-time process environment, so restored workers read this
	// annotation through the Downward API before creating DistributedRuntime.
	CheckpointRestoreRuntimeConfigAnnotation = "nvidia.com/dynamo-restore-runtime-config"
	...
	PodInfoFileDynRestoreRuntimeConfig  = "dyn_restore_runtime_config"
